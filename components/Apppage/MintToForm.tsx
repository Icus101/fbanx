...
export const MintToForm: FC = () => {
  ...
  const mintTo = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const form = event.target as typeof event.target & {
        mintPubKey: { value: string };
        recipientPubKey: { value: string };
        amount: { value: number };
      };
    ...
    const mintPubKey = new web3.PublicKey(form.mintPubKey.value);
    const recipientPubKey = new web3.PublicKey(form.recipientPubKey.value);
